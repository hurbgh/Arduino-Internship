  ...
  if (pmsConnect.available()){
    ...
    if (position==32){
      ...
      if (checkCode==sumForCheck){
        display.println(F("The data has good integrity. Check code success."));
        display.setCursor(0, 10);
      } else{
        display.println(F("The data has bad integrity. Check code failed."));
        display.setCursor(0, 10);
      }

      display.print(F("PM2.5 concentration unit μ g/m3 standard particle: "));
      display.println((dataArray[6]*256)+dataArray[7]);
      display.display();
      safety=false;
      position=1;
    }
